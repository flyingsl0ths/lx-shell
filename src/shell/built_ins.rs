pub mod cd {
    ...
    pub fn cmd(
        ...
        let on_no_path_given = || match home_dir {
        ...
        let on_path_given = |mut path: String| {
            if let Some(expanded_path) =
                super::expand_home_path(&path, home_dir)
            {
                path = expanded_path
            }
...
fn expand_home_path(path: &str, home_dir: Option<&String>) -> Option<String> {
    ...
    let home_dir = home_dir.unwrap().clone();
...
mod tests {
    mod cd {
        ...
        use super::super::{cd, expand_home_path};
        ...
        fn expands_when_path_contains_only_a_tilde() {
            let home_dir = "/home/flyingsloths".to_string();

            assert_eq!(
                Some(home_dir.to_string()),
                expand_home_path("~", Some(&home_dir))
            );
        }
        ...
        fn expands_when_path_contains_a_tilde() {
            ...
            assert_eq!(
                ...
                expand_home_path("~/.config/", Some(&home_dir))
            );
        }
        ...
        fn when_no_path_is_given_changes_to_home_dir() {
            let test_home_dir = |test_dir_path: &str| {
                ...
                if cd::cmd(None, Some(test_dir_path.to_string())).is_err() {
